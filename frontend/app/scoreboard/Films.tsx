  ...
  if (ended === false) {
    ...
    for (let i = 0; i < half; i++) {
      if (i === 0) {
        ...
        if (length % 2 === 0) {
          nonWinningFilms.push(
            <div style={{animationDelay: ((length-(i+half))/finalDelay).toString() + "s"}} className="film" key={i + half}>
          ...
          nonWinningFilms.push(
            <div style={{animationDelay: ((length-((i+half)))/finalDelay).toString() + "s"}} className="film" key={i + half + 1}>
